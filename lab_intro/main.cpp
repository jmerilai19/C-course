...
PNG brighten(PNG original, int amount)
{
    ...
    for (int yi = 0; yi < original.height(); yi++)
    {
		for (int xi = 0; xi < original.width(); xi++){
            original(xi, yi)->red = std::min(255, original(xi, yi)->red + amount);
            original(xi, yi)->green = std::min(255, original(xi, yi)->green + amount);
            original(xi, yi)->blue = std::min(255, original(xi, yi)->blue + amount);
	    }
    }
    return original;
}
